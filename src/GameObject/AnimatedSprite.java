...
public class AnimatedSprite implements IntersectableRectangle {
	...
	// current Frame object the animation is using based on currentAnimationName and
	// currentFrameIndex
	// this is essential for the class, as it uses this to be treated as "one
	// sprite"
	...
	public void update() {
		// if animation name has been changed (previous no longer equals current), setup
		// for the new animation and start using it
		if (!previousAnimationName.equals(currentAnimationName)) {
			...
			// if animation has more than one frame, check if it's time to transition to a
			// new frame based on that frame's delay
			if (getCurrentAnimation().length > 1 && currentFrame.getDelay() > 0) {

				// if enough time has passed based on current frame's delay and it's time to
				// transition to a new frame,
				// update frame index to the next frame
				// It will also wrap around back to the first frame index if it was already on
				// the last frame index (the animation will loop)
	...
	// Subclasses can override this method in order to add their own animations,
	...
	// gets the frame from current animation that the animated sprite class is
	// currently using
	...
	public boolean axeRange(IntersectableRectangle other, float x, float y) {
		float pX = getX();
		float pY = getY();
	...
	public String toString() {
		return String.format("Current Sprite: x=%s y=%s width=%s height=%s bounds=(%s, %s, %s, %s)", x, y, getWidth(),
				getHeight(), getBoundsX1(), getBoundsY1(), getBounds().getWidth(), getBounds().getHeight());
	}
}
